...
Es wird nur verschickt, wenn es seit der letzten Mail neue interessante Einträge gibt.
"""
...
def build_message(items: list[dict], recipients: list[str], test: bool = False) -> EmailMessage:
    regions = []
...
def run_digest(test: bool = False) -> dict:
    ...
    if not items and not test:
        log.info("Tägliche E-Mail: keine neuen interessanten Treffer seit %s", since)
    ...
    try:
        send(build_message(items, recipients, test=test))
